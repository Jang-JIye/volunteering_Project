...
public class EnrollmentServiceImpl implements EnrollmentService {
    ...
    @Override
    @Transactional
    public EnrollmentResponseDto attend(Long postId, EnrollmentRequestDto requestDto, UserDetailsImpl userDetails) {
...
/*        if (!enrollment.getUserId().equals(userId)) {
        ...
        Enrollment enrollment = new Enrollment(postId, requestDto, userDetails.getUserId(), post);
    ...
    @Override
    @Transactional
    public void cancel(Long postId, UserDetailsImpl userDetails, Long enrollmentId) {
        VolunteerWorkPost Post = volunteerWorkPostRepository.findByPostId(postId).orElseThrow(
                () -> new IllegalArgumentException("해당 게시글이 존재하지 않습니다.")
        );
 /*       if (!enrollment.getUserId().equals(userId)) {
        ...
        Enrollment enrollment = enrollmentRepository.findByEnrollmentId(enrollmentId).orElseThrow(
                () -> new IllegalArgumentException("참여신청 한 게시글이 아닙니다.")
        );
        //본인의 참여 신청이 아닌 경우
